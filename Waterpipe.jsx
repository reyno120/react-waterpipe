...
class Waterpipe extends Component {
    state = {  }

    componentDidMount() {
        var {
            gradientStart, gradientEnd, smokeOpacity, numCircles, maxMaxRad, minMaxRad,
             minRadFactor, iterations, drawsPerFrame, lineWidth, speed, bgColorInner, bgColorOuter
            } = this.props.options;

        new Smoke({
            gradientStart: gradientStart !== undefined ? gradientStart : '#000000',
            gradientEnd: gradientEnd !== undefined ? gradientEnd : '#222222',
            smokeOpacity: smokeOpacity !== undefined ? smokeOpacity : 0.1,
            numCircles: numCircles !== undefined ? numCircles : 1,
            maxMaxRad: maxMaxRad !== undefined ? maxMaxRad : 'auto',
            minMaxRad: minMaxRad !== undefined ? minMaxRad : 'auto',
            minRadFactor: minRadFactor !== undefined ? minRadFactor : 0,
            iterations: iterations !== undefined ? iterations : 8,
            drawsPerFrame: drawsPerFrame !== undefined ? drawsPerFrame : 10,
            lineWidth: lineWidth !== undefined ? lineWidth : 2,
            speed: speed !== undefined ? speed : 1,
            bgColorInner: bgColorInner !== undefined ? bgColorInner : "#ffffff",
            bgColorOuter: bgColorOuter !== undefined ? bgColorOuter : "#666666"
        });
    }

    render() { 
        return (  
            <div id="wavybg-wrapper" style={{width: '100%', height: '100vh'}}> 
                <canvas></canvas>
            </div>
        );
    }
...
class Smoke {
    constructor(options) {
        this.$element = document.querySelector("#wavybg-wrapper");
        inst = this;
    ...
    initSettings () {
        var radius = this.$element.offsetHeight*0.8/2; 
    ...
    hexToRGBA(hex, opacity) {
        hex = hex.replace('#','');
        var r = parseInt(hex.substring(0,2), 16);   // added var for r, g, b
        var g = parseInt(hex.substring(2,4), 16);
        var b = parseInt(hex.substring(4,6), 16);

        var result = 'rgba('+r+','+g+','+b+','+opacity+')'; // added var for result
        return result;
    }
}
